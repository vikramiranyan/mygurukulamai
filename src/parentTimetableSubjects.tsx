...
export function ParentTimetableSubjects({ children, active, setActive, driveSync }: {
  children: Child[];
  active: string;
  ...
  useEffect(() => {
    ...
    if (!child) return;
    void driveSync.loadTimetable(child.id).then(value => { if (!cancelled) setRecord(value); }).catch(error => {
  ...
  const processFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setNotice(`Timetable reading failed: ${error instanceof Error ? error.message : 'unknown PDF/image processing error'}`);
    } finally { setBusy(false); }
  };
  ...
  const addSubject = async () => {
    ...
    if (currentSubjects.some(s => s.toLocaleLowerCase() === subject.toLocaleLowerCase())) { setNotice('That subject already exists for this child.'); return; }
    if (!record && !draft) { setNotice('Upload and confirm a timetable before managing subjects.'); return; }
    const nextSubjects = uniqueSubjects([...currentSubjects, subject]); const now = new Date().toISOString(); setBusy(true);
    try {
      if (draft) setDraft({ ...draft, subjects: nextSubjects });
      else if (record) setRecord(await driveSync.updateSubjects(child.id, nextSubjects, { action: 'add_subject', at: now, subject }));
      setNewSubject(''); setNotice(`Subject “${subject}” added for ${child.name}.`);
  ...
  const modifySubject = async () => {
    ...
    const subjects = uniqueSubjects(currentSubjects.map(s => s === subjectEdit ? nextName : s)); const now = new Date().toISOString(); setBusy(true);
    try {
      if (draft) setDraft({ ...draft, periods, subjects });
      else if (record) setRecord(await driveSync.saveTimetable({ ...record, periods, subjects, audit: [...record.audit, { action: 'modify_subject', at: now, previousSubject: subjectEdit, newSubject: nextName }] }));
  ...
  const deleteSubject = async (subject: string) => {
    ...
    const subjects = currentSubjects.filter(s => s.toLocaleLowerCase() !== subject.toLocaleLowerCase()); const now = new Date().toISOString(); setBusy(true);
    try {
