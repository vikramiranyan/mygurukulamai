...
describe('Stage 3 curriculum acceptance', () => {
  ...
  it('supports the complete source-to-curriculum pipeline', () => {
    ...
    expect(book.confidence).toBeGreaterThanOrEqual(0.95);
    ...
    const contents = extractContentsEntries('1 Numbers ........ 5\n2 Addition ........ 17');
    const ranges = mapChapterPageRanges(contents, 30);
    expect(ranges).toEqual([
      { title: '1 Numbers', startPage: 5, endPage: 16 },
      { title: '2 Addition', startPage: 17, endPage: 30 }
    ]);

    const upload = createParentChapterUpload('mat-01', 'numbers.pdf', [5, 6, 7]);
    ...
    expect(parentSource.status).toBe('parent_uploaded');
    expect(canTeachFromSource(parentSource)).toBe(true);
  });
});
