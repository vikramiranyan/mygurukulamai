...
import { trustedSource } from '../core/curriculumWorkflow';
...
export function LearningHome({ child, onParents, signout }: Props) {
  ...
  const verified = trustedSource(chapter);
  ...
  const begin = () => {
    ...
    setAnswers([]); setQuestion(''); setAnswer(''); setFeedback('');
  };
