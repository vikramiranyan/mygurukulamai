...
function childId() {
  return `CHD-${crypto.randomUUID().replace(/-/g, '').slice(0, 12).toUpperCase()}`;
}

function blankChild(): Child {
  return { id: childId(), name: '', dob: '', gender: '', grade: '', section: '', school: '', board: '' };
}

function Brand({ child = false }: { child?: boolean }) {
  return <div className="gurukulam-brand"><img className="gurukulam-brand-icon" src="./icons/icon.svg" alt="Gurukulam AI" /><div className="brand"><strong>Gurukulam AI</strong><small>{child ? 'Personal AI Teacher' : 'Parents Dashboard'}</small></div></div>;
}
...
function ChildDashboard({ child, onParents, signout }: { child: Child; onParents: () => void; signout: () => void }) {
  return <div className="app dashboard-app"><header><Brand child /><div className="dashboard-actions"><button className="parent-access" onClick={onParents}>👨‍👩‍👧 Parent Dashboard</button><button className="dashboard-signout" onClick={signout}>⇥ Sign out</button></div></header><main><section className="child-welcome panel"><div><small>MY LEARNING SPACE</small><h1>Hello, {child.name || 'Student'}! 👋</h1><p>Welcome to your Gurukulam AI learning journey.</p></div><div className="child-id-badge">Child ID · {child.id}</div></section></main><footer>Gurukulam AI · Child learning environment</footer></div>;
}

function ChildAvatar({ gender }: { gender: string }) {
  if (gender === 'Female') return <span className="child-avatar child-avatar-female"><img src="./assets/female-child-avatar.svg" alt="Girl child" /></span>;
  if (gender === 'Male') return <span className="child-avatar child-avatar-male"><img src="./assets/male-child-avatar.svg" alt="Boy child" /></span>;
...
function ChildDetails({ children, setChildren, active, setActive, driveSync }: { children: Child[]; setChildren: React.Dispatch<React.SetStateAction<Child[]>>; active: string; setActive: (id: string) => void; driveSync: DriveSyncController }) {
  ...
  const saveChild = async () => {
    if (!edit) return;
    if (Object.values(edit).some((value, index) => index > 0 && !String(value).trim())) { setNotice('Please complete all child details before saving.'); return; }
...
function Parents({ children, setChildren, active, setActive, back, signout, driveSync }: { children: Child[]; setChildren: React.Dispatch<React.SetStateAction<Child[]>>; active: string; setActive: (id: string) => void; back: () => void; signout: () => void; driveSync: DriveSyncController }) {
...
function App() {
  ...
  useEffect(() => {
    if (!session) { driveSync.reset(); setChildren([]); setActive(''); return; }
    setChildren([]); setActive(''); setView('child');
    driveSync.configure(GOOGLE_CLIENT_ID, async () => { try { const remote = await driveSync.loadChildren(); setChildren(remote); setActive(remote[0]?.id || ''); } catch (error) { console.error(error); } }, error => console.error(error));
  ...
  return view === 'parents' ? <Parents children={children} setChildren={setChildren} active={active} setActive={setActive} back={() => setView('child')} signout={signout} driveSync={driveSync} /> : <ChildDashboard child={child} onParents={() => setView('parents')} signout={signout} />;
}
