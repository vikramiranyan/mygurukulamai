...
function Login({ setSession }: { setSession: (s: AuthSession) => void }) {
  useEffect(() => { let timer: number | undefined; const init = () => window.google?.accounts.id.initialize({ client_id: GOOGLE_CLIENT_ID, callback: response => { const session = credentialToSession(response.credential, GOOGLE_CLIENT_ID); if (session) setSession(session); } }); if (window.google) init(); else timer = window.setInterval(() => { if (window.google) { if (timer) window.clearInterval(timer); init(); } }, 100); return () => { if (timer) window.clearInterval(timer); }; }, [setSession]);
...
function Parents({ children, setChildren, active, setActive, back, signout, driveSync, workspace, setWorkspace }: { children: Child[]; setChildren: React.Dispatch<React.SetStateAction<Child[]>>; active: string; setActive: (id: string) => void; back: () => void; signout: () => void; driveSync: DriveSyncController; workspace: LearningWorkspace; setWorkspace: React.Dispatch<React.SetStateAction<LearningWorkspace>> }) {
  ...
  useEffect(() => { if (menu === 'subjects' || menu === 'teaching') void syncTimetable(); }, [menu, child?.id]);
...
function App() {
  ...
  useEffect(() => {
    ...
    // Persist the authenticated user before configuring Drive so the controller
    // can remember the user's prior drive.file grant across page reloads.
