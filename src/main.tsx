...
function App(){
 ...
 return <div className="app">
  ...
  <main>
   ...
   <section className="next panel"><div><small>AFTER APPROVAL</small><h3>{teaching ? `${teacher} is teaching ${childName}` : 'Ready for the Tutor Engine'}</h3><p>{teaching ? `Lesson: ${chapter}. ${answer==='correct'?'Excellent! Mastery check passed.':answer==='retry'?'Let’s try that again with a simpler explanation.':`Question: ${question}`}` : 'The verified chapter becomes the trusted source for teaching, practice, assessment and mastery tracking.'}</p></div><div className="actions"><button className="secondary" disabled={!teaching || !isApproved} onClick={()=>setAnswer('retry')}>I don't know</button><button className="primary" disabled={!isApproved} onClick={()=>{setTeaching(true);setAnswer(answer==='correct'?'correct':null)}}>{teaching ? 'Continue →' : 'Start teaching →'}</button></div></section>
   {teaching && isApproved && <section className="panel check"><small>KNOWLEDGE CHECK</small><h3>{question}</h3><div className="check-actions"><button onClick={()=>setAnswer('correct')}>✓ I know it</button><button onClick={()=>setAnswer('retry')}>↻ Need help</button></div></section>}
  </main>
