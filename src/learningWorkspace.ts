...
export type TeachingPlanItem = { id: string; subject: string; topic: string; duration: number; objective: string; completed: boolean };
...
export type ChildWorkspace = { teachers: TeacherProfile[]; subjects: string[]; tests: TestExam[]; today: TeachingPlanItem[]; homework: HomeworkItem[] };
...
export function defaultWorkspace(subjects: string[] = []): ChildWorkspace {
  ...
  return {
    ...
    subjects: unique,
    tests: [],
    today: unique.slice(0, 3).map((subject, i) => ({ id: `today-${i + 1}`, subject, topic: 'Next lesson', duration: 25, objective: `Build confidence in ${subject}.`, completed: false })),
    homework: [],
  };
}
...
export function normalizeWorkspace(value: unknown): LearningWorkspace {
  ...
  return value as LearningWorkspace;
}
