...
async function driveRequest<T>(token: string, url: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      try { detail = (await response.text()).slice(0, 500); } catch { /* Ignore secondary parsing failures. */ }
    }
...
export async function probeDriveAccess(token: string): Promise<void> {
  await driveRequest<{ user?: { permissionId?: string } }>(token, `${DRIVE_API}/about?fields=user(permissionId)`);
}
...
export async function ensureGurukulamFolders(token: string): Promise<{ rootId: string; childrenId: string }> {
  ...
  const result = { rootId: root.id, childrenId: children.id };
  folderCache.set(token, result);
  return result;
}

export function clearDriveFolderCache(token?: string): void {
  if (token) folderCache.delete(token);
...
export async function findChildFile(token: string, childrenFolderId: string, childId: string): Promise<DriveFile | null> {
  const q = encodeURIComponent(`'${escapeDriveQueryValue(childrenFolderId)}' in parents and name = '${escapeDriveQueryValue(childId)}.json' and trashed = false`);
  const data = await driveRequest<DriveListResponse>(token, `${DRIVE_API}/files?q=${q}&corpora=user&fields=files(id,name,mimeType,parents)&spaces=drive&pageSize=100`);
  return data.files[0] || null;
}
...
export async function deleteFile(token: string, fileId: string): Promise<void> {
  ...
  if (!response.ok) {
    let detail = '';
    try { const body = await response.json() as { error?: { message?: string; status?: string } }; detail = body.error?.message || body.error?.status || ''; }
