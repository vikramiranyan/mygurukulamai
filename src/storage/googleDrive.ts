...
async function driveRequest<T>(token: string, url: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    let detail = '';
    try { const body = await response.json() as { error?: { message?: string; status?: string; errors?: Array<{ reason?: string; message?: string }> } }; detail = body.error?.message || body.error?.status || body.error?.errors?.[0]?.reason || ''; }
    catch { try { detail = (await response.text()).slice(0, 500); } catch { /* Ignore secondary parsing failures. */ } }
    throw new DriveApiError(response.status, `Google Drive request failed (${response.status}${response.statusText ? ` ${response.statusText}` : ''})${detail ? `: ${detail}` : ''}`);
  }
...
export async function readFile<T>(token: string, fileId: string): Promise<T> { return driveRequest<T>(token, `${DRIVE_API}/files/${encodeURIComponent(fileId)}?alt=media`); }
export async function writeJson<T>(token: string, folderId: string, fileName: string, data: T, existingFileId?: string): Promise<DriveFile> {
  const body = JSON.stringify(data, null, 2); const metadata = JSON.stringify({ name: fileName, mimeType: 'application/json', ...(existingFileId ? {} : { parents: [folderId] }) });
