...
function hasStoredGrant(): boolean { try { return sessionStorage.getItem(DRIVE_GRANT_KEY) === '1'; } catch { return false; } }
function markStoredGrant(): void { try { sessionStorage.setItem(DRIVE_GRANT_KEY, '1'); } catch { /* storage may be unavailable */ } }
function clearStoredGrant(): void { try { sessionStorage.removeItem(DRIVE_GRANT_KEY); } catch { /* storage may be unavailable */ } }
  ...
  return google.accounts.oauth2.initTokenClient({
    ...
    callback: (response: DriveTokenResponse) => {
      if (response.access_token) {
        ...
        if (grantedScopes.length && !grantedScopes.includes(GOOGLE_DRIVE_SCOPE)) { clearStoredGrant(); onError?.(new Error('Google Drive permission was not granted for this app')); return; }
        markStoredGrant(); callback(response.access_token); return;
      }
      ...
      const error = new Error(detail || 'Google Drive authorization failed'); (error as Error & { code?: string }).code = code;
      if (hasStoredGrant() && code && SILENT_RECOVERY_ERRORS.has(code)) {
        clearStoredGrant();
        try {
          const retryClient = google.accounts.oauth2.initTokenClient({ client_id: clientId, scope: GOOGLE_DRIVE_SCOPE, include_granted_scopes: true, callback: (retry: DriveTokenResponse) => { if (retry.access_token) { markStoredGrant(); callback(retry.access_token); return; } onError?.(error); } });
          retryClient.requestAccessToken({ prompt: '' }); return;
        } catch { /* fall through */ }
      }
      onError?.(error);
    },
...
export function requestDriveAccess(client: DriveTokenClient, prompt?: DrivePrompt) {
  const effectivePrompt: DrivePrompt = prompt === 'consent' || prompt === 'select_account' ? prompt : (hasStoredGrant() ? 'none' : '');
