...
export type ChildTimetableAudit = { action: 'upload' | 'confirm' | 'add_subject' | 'modify_subject' | 'delete_subject'; at: string; subject?: string; previousSubject?: string; newSubject?: string };
export type ChildTimetableRecord = { version: 1; childId: string; fileName: string; fileMimeType: string; fileSize: number; originalDriveFileId?: string; uploadedAt: string; status: 'uploaded' | 'reviewed' | 'confirmed'; periods: TimetablePeriodRecord[]; subjects: string[]; audit: ChildTimetableAudit[] };
...
export async function saveChildTimetable(token: string, record: ChildTimetableRecord): Promise<ChildTimetableRecord> {
  ...
  if (!record.periods.length && record.fileName !== 'manual-subjects') throw new Error('At least one timetable period is required.');
  const { childrenId } = await ensureGurukulamFolders(token); const existing = await findTimetableFile(token, record.childId);
  const safeRecord: ChildTimetableRecord = { ...record, childId: record.childId.trim(), subjects: uniqueSubjects(record.subjects), periods: record.periods.map(period => ({ ...period, day: period.day.trim(), start: period.start.trim(), end: period.end.trim(), subject: normalizeSubject(period.subject) })) };
  await writeJson(token, childrenId, timetableFileName(safeRecord.childId), safeRecord, existing?.id); return safeRecord;
}
export async function updateChildSubjects(token: string, childId: string, subjects: string[], auditEntry: ChildTimetableRecord['audit'][number]): Promise<ChildTimetableRecord> {
  const current = await loadChildTimetable(token, childId); if (!current) throw new Error('No confirmed timetable exists for this child yet.');
  const next = { ...current, subjects: uniqueSubjects(subjects), audit: [...current.audit, auditEntry] }; await saveChildTimetable(token, next); return next;
}
