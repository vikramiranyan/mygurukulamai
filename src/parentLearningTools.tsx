...
export function ParentLearningTools({ mode, children, active, setActive, workspace, setWorkspace }: Props) {
  ...
  useEffect(() => {
    if (!workspace.subjects.includes(subject)) setSubject(workspace.subjects[0] || '');
  ...
  const saveTeacher = () => {
    ...
    const teacher: TeacherProfile = { id: `teacher-${name.toLowerCase().replace(/\s+/g, '-')}`, name, role: 'Personal AI Teacher', subjects: subject ? [subject] : workspace.subjects.slice(0, 3), style: 'Warm, patient and step-by-step', enabled: true };
    const others = workspace.teachers.filter(t => t.id !== teacher.id);
    setWorkspace({ ...workspace, teachers: [...others, teacher] }); setText('');
  };

  const addSubject = () => {
    const value = text.trim();
    if (!value || workspace.subjects.some(s => s.toLowerCase() === value.toLowerCase())) return;
    setWorkspace({ ...workspace, subjects: [...workspace.subjects, value] });
    setSubject(value); setText('');
  };

  const removeSubject = (value: string) => setWorkspace({
    ...workspace,
    subjects: workspace.subjects.filter(s => s !== value),
    chapters: workspace.chapters.filter(chapter => chapter.subject !== value),
    today: workspace.today.filter(item => item.subject !== value),
    homework: workspace.homework.filter(item => item.subject !== value),
    tests: workspace.tests.filter(item => item.subject !== value),
  });
  ...
  const uploadChapter = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const title = window.prompt(`Chapter name for ${subject}`, file.name.replace(/\.[^.]+$/, '').replace(/[_-]+/g, ' ').trim());
    if (!title?.trim()) return;
    setBusy(true); setNotice('Reading chapter pages…');
    try {
      const pages = await extractChapterPages(file);
      if (!pages.length) throw new Error('No pages could be read from this file.');
      const chapter: ChapterRecord = { id: crypto.randomUUID(), subject, title: title.trim(), fileName: file.name, uploadedAt: new Date().toISOString(), pages };
      setWorkspace({ ...workspace, chapters: [...workspace.chapters, chapter] });
      setChapterId(chapter.id); setPageNumbers([]); setNotice(`${chapter.title} uploaded under ${subject} with ${pages.length} page${pages.length === 1 ? '' : 's'}.`);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : 'Chapter could not be read.');
    } finally { setBusy(false); }
  };

  const deleteChapter = (chapter: ChapterRecord) => {
    if (!window.confirm(`Delete “${chapter.title}” and all of its page data?`)) return;
    setWorkspace({
      ...workspace,
      chapters: workspace.chapters.filter(item => item.id !== chapter.id),
      today: workspace.today.filter(item => item.chapterId !== chapter.id),
    });
    if (chapterId === chapter.id) setChapterId('');
    setNotice(`Chapter “${chapter.title}” deleted.`);
  };
  ...
  const addTeaching = () => {
    if (!subject || !selectedChapter) { setNotice('Select a subject and chapter first.'); return; }
    if (scope === 'pages' && !pageNumbers.length) { setNotice('Select at least one page, or choose Full chapter.'); return; }
    const orderedPages = [...pageNumbers].sort((a, b) => a - b);
    const target = scope === 'full_chapter' ? `Full chapter · ${selectedChapter.title}` : `Pages ${orderedPages.join(', ')} · ${selectedChapter.title}`;
    const item: TeachingPlanItem = {
      id: crypto.randomUUID(), subject, topic: target, duration: 25,
      objective: scope === 'full_chapter' ? `Learn and practise the full ${selectedChapter.title} chapter.` : `Learn and practise the selected pages from ${selectedChapter.title}.`,
      completed: false, scope, chapterId: selectedChapter.id, pageNumbers: scope === 'pages' ? orderedPages : undefined,
    };
    setWorkspace({ ...workspace, today: [...workspace.today, item] });
    setNotice(`Added ${target} to Today's Teaching.`);
  };
  ...
  return <section className="parent-section">
    <div className="section-heading"><div><small>PARENT CONTROL</small><h1>{mode === 'teachers' ? '👨‍🏫 Teacher Details' : mode === 'subjects' ? '📚 Subjects & Chapters' : mode === 'tests' ? '📝 Test / Exam' : mode === 'teaching' ? "📖 Today's Teaching" : "🏠 Kid's Homework"}</h1><p>Configure learning for <strong>{child.name}</strong>. Changes apply only to this child.</p></div>{selectChild}</div>
    ...
    {mode === 'subjects' && <>
      <div className="panel"><h2>Subjects</h2><p>Subjects are taken from the confirmed timetable. Add/modify/delete remains available in Time Table / Subjects.</p><div className="subject-add-row"><input value={text} onChange={e => setText(e.target.value)} placeholder="Add another subject"/><button className="primary" onClick={addSubject}>＋ Add Subject</button></div></div>
      <div className="subject-list">{workspace.subjects.map(value => <article className="panel" key={value} style={{ marginBottom: 14 }}><div className="section-heading"><div><h2>📘 {value}</h2><p>{workspace.chapters.filter(chapter => chapter.subject === value).length} chapter(s)</p></div><button className="danger" onClick={() => removeSubject(value)}>Delete Subject</button></div>
        <div className="subject-add-row"><label className="upload-button">{busy ? 'Reading…' : '＋ Upload Chapter'}<input type="file" accept=".pdf,.png,.jpg,.jpeg" disabled={busy} onChange={uploadChapter}/></label></div>
        {workspace.chapters.filter(chapter => chapter.subject === value).map(chapter => <details key={chapter.id} className="subject-row" open={chapter.id === chapterId}><summary><strong>📖 {chapter.title}</strong><span>{chapter.pages.length} pages · {chapter.fileName}</span></summary><div style={{ display: 'grid', gap: 8, marginTop: 12 }}>{chapter.pages.map(page => <div key={page.number} className="panel" style={{ padding: 12, background: 'var(--panel-soft, #f6f6f6)' }}><strong>Page {page.number}</strong><p style={{ whiteSpace: 'pre-wrap', marginBottom: 0 }}>{page.text || 'No readable text detected on this page.'}</p></div>)}<button className="danger" onClick={() => deleteChapter(chapter)}>Delete Chapter</button></div></details>)}
        {!workspace.chapters.some(chapter => chapter.subject === value) && <p>No chapters uploaded yet. Upload the textbook chapter PDF/image for this subject.</p>}
      </article>)}</div>
    </>}
    ...
    {mode === 'teaching' && <>
      <div className="panel"><h2>Plan today's teaching</h2><div className="form-grid"><label>Subject<select value={subject} onChange={e => { setSubject(e.target.value); setChapterId(''); setPageNumbers([]); }}>{workspace.subjects.map(s => <option key={s}>{s}</option>)}</select></label><label>Chapter<select value={chapterId} onChange={e => { setChapterId(e.target.value); setPageNumbers([]); }}><option value="">Select chapter</option>{subjectChapters.map(chapter => <option key={chapter.id} value={chapter.id}>{chapter.title}</option>)}</select></label><label>Teaching scope<select value={scope} onChange={e => { setScope(e.target.value as TeachingScope); setPageNumbers([]); }}><option value="full_chapter">Full chapter</option><option value="pages">Specific pages</option></select></label></div>
        {selectedChapter && scope === 'pages' && <div className="panel" style={{ marginTop: 14 }}><h3>Select pages from {selectedChapter.title}</h3><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: 8 }}>{selectedChapter.pages.map(page => <label key={page.number} className="subject-row" style={{ cursor: 'pointer' }}><input type="checkbox" checked={pageNumbers.includes(page.number)} onChange={event => setPageNumbers(current => event.target.checked ? [...current, page.number] : current.filter(number => number !== page.number))}/><strong>Page {page.number}</strong></label>)}</div></div>}
        {!subjectChapters.length && <p className="tt-notice">No chapters uploaded for this subject yet. Upload a chapter under Subjects before creating today's teaching.</p>}
        <button className="primary" disabled={busy || !selectedChapter || (scope === 'pages' && !pageNumbers.length)} onClick={addTeaching}>＋ Add to Today's Teaching</button>
      </div>
