...
export class BrowserVoice {
 ...
 startSTT(language='en-IN',onResult:(r:SpeechResult)=>void,onError:(e:VoiceError)=>void):boolean{const C=(window as any).SpeechRecognition||(window as any).webkitSpeechRecognition;if(!C){onError('unsupported');return false;}const r=new C();this.recognition=r;r.lang=language;r.interimResults=true;r.continuous=false;r.onresult=(e:any)=>{const x=e.results[e.results.length-1];onResult({text:x[0].transcript,confidence:x[0].confidence,final:x.isFinal});};r.onerror=(e:any)=>onError(e.error==='not-allowed'?'permission-denied':e.error==='no-speech'?'no-speech':e.error==='network'?'network':'unknown');try{r.start();return true;}catch{onError('unknown');return false;}}
