...
function mapVoiceError(error:any):VoiceError{const code=String(error?.error??error?.name??'').toLowerCase();const message=String(error?.message??'').toLowerCase();if(code==='not-allowed'||code==='permission-denied'||code==='securityerror'||message.includes('not allowed')||message.includes('permission'))return'permission-denied';if(code==='service-not-allowed')return'service-not-allowed';if(code==='audio-capture')return'audio-capture';if(code==='no-speech')return'no-speech';if(code==='network')return'network';if(code==='aborted'||code==='interrupted')return'aborted';return'unknown';}
