(() => {
  ...
  function createTeacher(scene) {
    ...
    const skin = material(0xc9825f);
    const skinLight = material(0xf0b08a);
    const hair = material(0x241b1c, 0.9);
    const blouse = material(0x2d7665);
    const sari = material(0xe995b0);
    const gold = material(0xd6a72c, 0.42, 0.35);
    const dark = material(0x172d2a);
    const shoe = material(0x253f3a);
    const mouthMat = material(0x7f3040);

    const hips = new THREE.Group();
    hips.position.y = 1;
    root.add(hips);

    const lower = new THREE.Mesh(new THREE.CylinderGeometry(0.56, 0.72, 1.15, 24), sari);
    lower.position.y = -0.25;
    hips.add(lower);
    const torso = new THREE.Mesh(new THREE.CapsuleGeometry(0.55, 0.82, 6, 18), blouse);
    ...
    const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.19, 0.22, 0.34, 18), skinLight);
    neck.position.y = 1.45;
    hips.add(neck);
    const head = new THREE.Group();
    head.position.y = 1.9;
    hips.add(head);
    const face = new THREE.Mesh(new THREE.SphereGeometry(0.62, 24, 18), skinLight);
    face.scale.set(0.92, 1.05, 0.88);
    head.add(face);
    const hairCap = new THREE.Mesh(new THREE.SphereGeometry(0.66, 24, 16, 0, Math.PI * 2, 0, Math.PI * 0.62), hair);
    ...
    head.add(braid);
    for (let i = 0; i < 7; i += 1) {
      const bead = new THREE.Mesh(new THREE.SphereGeometry(Math.max(0.08, 0.16 - i * 0.012), 12, 10), hair);
      bead.position.y = -i * 0.16;
      bead.position.z = 0.02 + i * 0.008;
      braid.add(bead);
    }
    ...
    head.add(eyeL, eyeR);
    const bindi = new THREE.Mesh(new THREE.SphereGeometry(0.045, 10, 8), material(0x9d2945));
    bindi.position.set(0, 0.29, 0.58);
    head.add(bindi);
    const nose = new THREE.Mesh(new THREE.ConeGeometry(0.07, 0.17, 12), skin);
    nose.rotation.x = Math.PI / 2;
    nose.position.set(0, -0.02, 0.58);
    head.add(nose);
    const mouth = new THREE.Mesh(new THREE.SphereGeometry(0.12, 18, 10), mouthMat);
    mouth.scale.set(1.35, 0.34, 0.42);
    mouth.position.set(0, -0.24, 0.57);
    head.add(mouth);
    const earringL = new THREE.Mesh(new THREE.TorusGeometry(0.09, 0.022, 6, 14), gold);
    ...
    const armGeo = new THREE.CapsuleGeometry(0.13, 0.7, 5, 12);
    ...
    armL.add(armMeshL);
    armR.add(armMeshR);
    const handGeo = new THREE.SphereGeometry(0.17, 14, 10);
    ...
    const legGeo = new THREE.CapsuleGeometry(0.16, 0.72, 5, 12);
    ...
    root.add(legL, legR);
    const shoeL = new THREE.Mesh(new THREE.SphereGeometry(0.24, 14, 10), shoe);
    ...
    const book = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.12, 0.38), material(0x5b66c9));
  ...
  function init(container) {
    ...
    const state = { destroyed: false, t: 0, active: true };
    ...
    const visibilityObserver = typeof IntersectionObserver === 'function' ? new IntersectionObserver((entries) => { state.active = entries[0]?.isIntersecting !== false; }, { threshold: 0 }) : null;
    ...
    const animate = (now) => {
      ...
      const dt = Math.min(0.05, Math.max(0.001, (now - (state.lastNow || now)) / 1000));
      state.lastNow = now;
      state.t += dt;
      const guide = container.closest('.ga-guide');
      const speaking = Boolean(window.speechSynthesis?.speaking);
      const celebrating = Boolean(guide?.classList.contains('celebrate'));
      ...
      teacher.armR.rotation.z = 0.22 + (celebrating ? -0.95 : 0) + Math.sin(state.t * 3.2) * 0.08 * motion;
      teacher.armL.rotation.z = -0.22 + (celebrating ? 0.55 : 0) + Math.sin(state.t * 2.5 + 1) * 0.04 * motion;
      teacher.book.rotation.y = Math.sin(state.t * 1.8) * 0.08 * motion;
      teacher.mouth.scale.y = 0.34 + (speaking ? 0.32 + Math.abs(Math.sin(state.t * 13)) * 0.5 : 0);
      teacher.mouth.scale.x = 1.35 + (speaking ? Math.abs(Math.sin(state.t * 9)) * 0.35 : 0);
